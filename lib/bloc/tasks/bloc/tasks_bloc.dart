...
import 'package:mytaskmanager/data/repositories/repositories.dart';
...
class TaskBloc extends Bloc<TaskEvent, TaskState> {
  ...
  Stream<TaskState> _mapWatchTaskByStatusToState(
      WatchTaskByStatus event) async* {
    final Stream<TaskWithCategoryEntity> entity =
        _taskRepository.watchAllTaskByStatus(event.statusType);
    yield TaskStream(entity: entity);
  }

  Stream<TaskState> _mapWatchTaskByDateToState(WatchTaskByDate event) async* {
    final Stream<TaskWithCategoryEntity> entity =
        _taskRepository.watchAllTaskByDate(event.dateTime);
    yield TaskStream(entity: entity);
  }

  Stream<TaskState> _mapWatchOnGoingTaskToState(WatchOnGoingTask event) async* {
    final Stream<TaskWithCategoryEntity> entity =
        _taskRepository.watchOnGoingTasks();
    yield OnGoingTaskStream(entity: entity);
  }

  Stream<TaskState> _mapWatchCompletedTaskToState(
      WatchCompletedTask event) async* {
    final Stream<TaskWithCategoryEntity> entity =
        _taskRepository.watchCompletedTasks();
